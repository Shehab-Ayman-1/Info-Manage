...
export const ListItem = ({ Icon, title, href, role }: NavLinkType) => {
    ...
    const itemStyle = "flex-between cursor-pointer border-none p-4 hover:bg-primary-50 dark:hover:bg-slate-800";
    const titleStyle = cn("text-base font-bold", !isAccessible ? "!text-rose-500" : "!text-slate-600 dark:!text-slate-400");
        ...
        <AccordionItem asChild value={title} key={title} className={itemStyle} onClick={() => onOpen(false)}>
            <Link href={!isAccessible ? "/" : href} className={cn(!isAccessible && "pointer-events-auto")}>
                <div className="flex-start">
                    <Icon className={cn("size-5", !isAccessible ? "!text-rose-500" : "!text-slate-600 dark:!text-slate-400")} />
                    <h3 className={titleStyle}>{title}</h3>
                </div>
                <LockIcon className={cn(isAccessible ? "hidden" : "!text-rose-500")} />
            </Link>
        </AccordionItem>
    );
};
