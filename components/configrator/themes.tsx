...
export const Themes = ({ setTheme }: ThemesProps) => {
    return (
        <div className="my-6">
            <h1 className="text-2xl font-bold text-primary">Theme Color</h1>
            <div className="flex-between my-8 flex-wrap !gap-6 px-4">
                {themes1.map(({ name, color }) => (
                    <SheetClose key={name} asChild>
                        <p
                            className={cn(
                                "size-6 cursor-pointer rounded-full border-2 border-black/50 hover:opacity-80 dark:border-white/20",
